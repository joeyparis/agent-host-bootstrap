...
bitbucket_mcp_env_file() {
  echo "${HOME}/.config/agentctl/mcp/bitbucket.env"
}

ensure_bitbucket_mcp_shell_env() {
  # Write an env file sourced by /etc/profile.d/agent-mcp.sh (installed by bootstrap).
  # This function writes no output besides warnings.
  local env_file
  env_file="$(bitbucket_mcp_env_file)"

  mkdir -p "$(dirname "$env_file")"
  chmod 0700 "$(dirname "$env_file")" 2>/dev/null || true

  cat >"$env_file" <<'EOF'
# Auto-generated by agentctl sync-config.
# This file may contain secrets. Permissions should be 0600.
EOF

  chmod 0600 "$env_file" 2>/dev/null || true
}

write_bitbucket_mcp_env_from_secret() {
  local region="$1"
  local config_name="$2"

  local secret_id="agent-host/${config_name}/bitbucket_mcp_credentials"
...
print("# Auto-generated by agentctl sync-config")
exports = {}
...
ensure_codex_mcp_config() {
  # Create/append Codex MCP config (secrets are NOT stored here; env vars are forwarded).
  ...
  if ! grep -q "^\[mcp_servers\.bitbucket\]$" "$cfg_file" 2>/dev/null; then
    cat >>"$cfg_file" <<'EOF'
...
ensure_gemini_mcp_config() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    jq '
     ...
     ' "$cfg_file" >"$tmp" && mv "$tmp" "$cfg_file" || rm -f "$tmp"
  fi
...
detect_aws_region() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    python3 -c 'import json,sys; print(json.loads(sys.argv[1]).get("region", ""))' "$doc" 2>/dev/null
    return 0
  fi
...
EOF
  ...
  # 3) Bitbucket MCP credentials (optional).
  if write_bitbucket_mcp_env_from_secret "$region" "$config_name"; then
    ensure_codex_mcp_config || true
    ensure_gemini_mcp_config || true
    echo "  - MCP env: $(bitbucket_mcp_env_file) (from agent-host/${config_name}/bitbucket_mcp_credentials)"
  else
    echo "  - MCP env: (not configured; missing agent-host/${config_name}/bitbucket_mcp_credentials)"
  fi

  # 4) Persist last-used config name for convenience.
...
create_worktree() {
  ...
  # Claude Code: project-scoped MCP config. Uses env var expansion.
  # This file is untracked and excluded from git status.
  if [[ -d "$workdir/.git" || -f "$workdir/.git" ]]; then
    mkdir -p "$workdir/.git/info" 2>/dev/null || true
    if [[ -f "$workdir/.git" ]]; then
      # Worktree .git is a file pointing to gitdir: <path>
      gitdir="$(awk -F': ' '/^gitdir:/ {print $2}' "$workdir/.git" 2>/dev/null)"
      if [[ -n "${gitdir:-}" ]]; then
    ...
    if [[ -n "${exclude_file:-}" ]]; then
      grep -qxF '.mcp.json' "$exclude_file" 2>/dev/null || echo '.mcp.json' >> "$exclude_file"
    fi
  fi
